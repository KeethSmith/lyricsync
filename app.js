...
async function showSpotifyAccount(){const box=$('accountStatus');box.hidden=false;if(!(token?.scope||'').split(' ').includes('user-read-private')){box.textContent='Spotify connected · Sign in again to verify the account name';return;}try{const profile=await spotifyRequest('/me');if(!profile)return;box.replaceChildren();const label=document.createElement('span');label.textContent=`Connected as ${profile.display_name||profile.id} (@${profile.id})`;box.append(label);if(profile.external_urls?.spotify){const link=document.createElement('a');link.href=profile.external_urls.spotify;link.target='_blank';link.rel='noopener';link.textContent='View Spotify profile ↗';box.append(link);}}catch(error){box.textContent=error.message;}}
