...
async function getLyrics(item,version){lines=[];empty('Finding the words…','Looking for lyrics for this recording.');try{const data=await lookupLyrics(item,fetch,rejectedVersions(item.id),rejectedLyrics(item.id));if(version!==generation)return;if(!data){empty('No more matching lyrics.','No usable versions remain for this recording. You can reset skipped versions to try them again.');return;}displayedLyrics=data;$('rejectLyrics').disabled=reportBusy||!Number.isSafeInteger(data.id);$('rejectLyrics').textContent='Wrong lyrics — report & try another';if(data.instrumental){empty('Let the music speak.','This track is marked as instrumental.');displayedLyrics=data;$('rejectLyrics').disabled=reportBusy||!Number.isSafeInteger(data.id);$('rejectLyrics').textContent='Report as not instrumental';return;}lines=parseLrc(data.syncedLyrics||'');if(lines.length){$('lyricSource').textContent='SYNCED · LRCLIB';renderLyrics();}else if(data.plainLyrics){$('lyricSource').textContent='UNSYNCED · LRCLIB';$('lyrics').replaceChildren(...data.plainLyrics.split('\n').map(text=>{const p=document.createElement('p');p.className='line';p.textContent=text||' ';return p;}));}else empty('No lyrics available.','LRCLIB has no lyrics for this recording.');}catch(err){if(version===generation)empty('Could not load lyrics.',err.message);}}
