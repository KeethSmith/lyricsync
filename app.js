...
$('connectForm').addEventListener('submit',async e=>{e.preventDefault();try{const client=SPOTIFY_CLIENT_ID;if(!/^[a-f\d]{32}$/i.test(client))throw Error('Spotify sign-in is not configured. You can explore the demo in the meantime.');const verifier=random(),state=random();sessionStorage.setItem(keys.auth,JSON.stringify({verifier,state}));const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(verifier));const challenge=btoa(String.fromCharCode(...new Uint8Array(digest))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');location.assign('https://accounts.spotify.com/authorize?'+new URLSearchParams({client_id:client,response_type:'code',redirect_uri:redirect,scope:'user-read-private user-read-currently-playing user-read-playback-state user-modify-playback-state',code_challenge_method:'S256',code_challenge:challenge,state}));}catch(err){status(err.message);}});
async function spotifyRequest(path){const res=await fetch('https://api.spotify.com/v1'+path,{headers:{Authorization:`Bearer ${token.access_token}`}});if(res.status===204)return null;if(res.status===429){cooldown=Date.now()+Math.max(5,Number(res.headers.get('Retry-After'))||30)*1000;throw Error('Spotify is busy. Waiting before checking again.');}if(res.status===401){token=null;sessionStorage.removeItem(keys.token);$('setup').hidden=false;throw Error('Your Spotify session expired. Connect again.');}if(res.status===403)throw Error('This Spotify account is not authorized for this Development Mode app. Add its Spotify email under User Management; the account must also have Premium.');if(!res.ok)throw Error('Spotify is temporarily unavailable. We will try again.');return res.json();}
function setSignedInLabel(account='Spotify'){$('signIn').textContent=`Logged in as ${account}`;$('signIn').disabled=true;$('signIn').classList.add('connected');$('logout').hidden=false;}
async function showSpotifyAccount(){const box=$('accountStatus');box.hidden=false;if(!(token?.scope||'').split(' ').includes('user-read-private')){box.textContent='Spotify connected · Sign in again to verify the account name';setSignedInLabel();return;}try{const profile=await spotifyRequest('/me');if(!profile)return;const account=`${profile.display_name||profile.id} (@${profile.id})`;setSignedInLabel(account);box.replaceChildren();const label=document.createElement('span');label.textContent=`Connected as ${account}`;box.append(label);if(profile.external_urls?.spotify){const link=document.createElement('a');link.href=profile.external_urls.spotify;link.target='_blank';link.rel='noopener';link.textContent='View Spotify profile ↗';box.append(link);}}catch(error){box.textContent=error.message;setSignedInLabel();}}
...
function renderLyrics(version=generation){
  selected=-2;
  ...
  const rendered=lines.map(line=>{
    ...
    if(!line.words){p.textContent=romanizeKorean(line.text);displayLyrics(line.text).then(text=>{if(version===generation&&p.isConnected)p.textContent=text;});return p;}
    const words=line.words.map(word=>{const span=document.createElement('span');span.className='lyric-word';span.textContent=romanizeKorean(word.text);displayLyrics(word.text).then(text=>{if(version===generation&&span.isConnected)span.textContent=text;});return span;});
    p.append(...words);
    return p;
  });
  if(version===generation)$('lyrics').replaceChildren(...rendered);
}
...
function reset(){generation++;clearTimeout(timer);token=null;sessionStorage.removeItem(keys.token);demo=false;playing=false;track=null;activeDevice=null;lines=[];base=0;setTrack({name:'Your next favorite moment',artists:[{name:'Start a song in Spotify'}],duration_ms:0});track=null;$('setup').hidden=false;$('accountStatus').hidden=true;$('accountStatus').replaceChildren();$('signIn').replaceChildren(document.createTextNode('Sign in with Spotify '),Object.assign(document.createElement('span'),{textContent:'↗'}));$('signIn').disabled=false;$('signIn').classList.remove('connected');$('logout').hidden=true;$('mode').textContent='NOW PLAYING';$('lyricSource').textContent='LRCLIB';empty('Every song has a story.','Connect Spotify to follow along.');status('Logged out.');}
$('settings').onclick=()=>{$('setup').hidden=!$('setup').hidden;if(!$('setup').hidden)$('signIn').focus();};
$('disconnect').onclick=reset;
$('logout').onclick=reset;
