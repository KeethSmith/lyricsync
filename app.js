...
$('rejectLyrics').onclick=async()=>{
  ...
  try{
    ...
    clearInterval(reportProgressTimer);reportProgressTimer=null;reportBusy=false;$('rejectLyrics').classList.remove('reporting');$('rejectLyrics').removeAttribute('aria-busy');$('rejectLyrics').style.removeProperty('--report-progress');$('rejectLyrics').textContent='Wrong lyrics — report & try another';$('rejectLyrics').disabled=demo||!Number.isSafeInteger(displayedLyrics?.id);
  }
};
