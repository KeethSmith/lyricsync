...
async function poll(force=false){clearTimeout(timer);if(busy||controlBusy||demo||!token)return;if(Date.now()<cooldown){timer=setTimeout(()=>poll(),cooldown-Date.now());return;}busy=true;const version=generation;try{const data=await spotify();if(version!==generation||demo)return;activeDevice=data?.device||null;if(!data?.item||data.item.type!=='track'){playing=false;track=null;lines=[];generation++;$('mode').textContent='WAITING FOR SPOTIFY';$('title').textContent='Nothing playing';$('artist').textContent='Start a song in Spotify';$('trackLink').hidden=true;$('art').hidden=true;$('artPlaceholder').hidden=false;empty('Waiting for your music.','Start a song in Spotify on any active device.');status('No active song. Open Spotify and press play.');return;}base=data.progress_ms||0;sampled=performance.now();playing=data.is_playing;$('mode').textContent=playing?'NOW PLAYING':'PAUSED';status(activeDevice?.is_restricted?`${activeDevice.name||'This device'} is playing, but Spotify does not allow remote playback controls on it.`:playing?'Connected · Following your Spotify playback':'Connected · Playback paused');if(track?.id!==data.item.id||force){setTrack(data.item);getLyrics(data.item,++generation);}}catch(err){playing=false;status(err.message);}finally{busy=false;if(token&&!demo)timer=setTimeout(()=>poll(),Math.max(5000,cooldown-Date.now()));}}
