...
setInterval(()=>{let pos=track?Math.min(track.duration_ms,base+(playing?performance.now()-sampled:0)):0;if(demo&&pos>=32000){sampled=performance.now();pos=0;}$('position').textContent=fmt(pos);$('bar').style.width=(track?.duration_ms?pos/track.duration_ms*100:0)+'%';updateControls(pos);if(!lines.length)return;const next=activeLine(lines,pos+Number($('offset').value)*1000);if(next!==selected){selected=next;[...$('lyrics').children].forEach((el,i)=>{el.classList.toggle('active',i===next);el.classList.toggle('past',i<next);if(i===next)el.setAttribute('aria-current','true');else el.removeAttribute('aria-current');});const el=$('lyrics').children[next];if(el&&$('follow').checked)$('lyrics').scrollTo({top:el.offsetTop-$('lyrics').clientHeight/2+el.clientHeight/2,behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});}},250);
