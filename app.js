...
async function renderLyrics(version=generation){
  selected=-2;
  ...
  const rendered=await Promise.all(lines.map(async line=>{
    ...
    if(!line.words){p.textContent=await displayLyrics(line.text);return p;}
    const words=await Promise.all(line.words.map(async word=>{const span=document.createElement('span');span.className='lyric-word';span.textContent=await displayLyrics(word.text);return span;}));
    p.append(...words);
    return p;
  }));
