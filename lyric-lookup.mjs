...
export async function lookupLyrics(item,request=fetch,excluded=[]){
  ...
  try{
    ...
    const candidates=result.ok?(await result.json()).filter(candidate=>!excluded.includes(candidate.id)):[];
    const broadCandidates=broadResult.ok?(await broadResult.json()).filter(candidate=>!excluded.includes(candidate.id)):[];
