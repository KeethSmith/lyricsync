...
export async function lookupLyrics(item,request=fetch){
  ...
  const exact=response.ok?await response.json():null;
  ...
  try{
    ...
    return selectTimedMatch(await result.json(),item)||exact;
  }catch{return exact;}
}
